...
public class BangTinFragment extends Fragment implements NewsPostAdapter.OnItemClickListener {
    ...
    private static final String IMAGE_BASE_URL = "http://192.168.114.213:5089"; // THAY BẰNG IP CỦA BẠN
            ...
            public void onResponse(Call<List<BaiViet>> call, Response<List<BaiViet>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (BaiViet bv : response.body()) {
                        String imageUrl = (bv.hinhAnh != null && !bv.hinhAnh.isEmpty()) ? IMAGE_BASE_URL + bv.hinhAnh : null;
                        boolean showDonation = bv.idNguoiDang == 2;
                        allPosts.add(new NewsPost(bv.id, "Người dùng " + bv.idNguoiDang, bv.ngayDang, bv.noiDung, imageUrl, R.drawable.ic_launcher_background, bv.soLuotThich, bv.soLuotBinhLuan, showDonation, 25, 120, 7));
                    }
    ...
    private void createPostOnServer(int idNguoiDang, String noiDung, @Nullable Uri imageUri, boolean isCallingForDonation) {
        ...
        RequestBody isCallingPart = RequestBody.create(MediaType.parse("text/plain"), String.valueOf(isCallingForDonation));
